...
class EncoderBridge(nn.Module):
    ...
    def __init__(self, hidden_size: int, num_source_features: int, num_decoder_layers: int):
        super().__init__()
        ...
        if self.num_source_features > 0:
            self.input2features = nn.Sequential(nn.Linear(self.hidden_size, self.num_source_features), nn.Sigmoid())
    ...
    def forward(self, sequences: Tensor, mask: Tensor) -> BridgeOutput:
        ...
        masked_sequences = torch.masked_fill(sequences, mask=mask, value=self.neg_inf_score)
